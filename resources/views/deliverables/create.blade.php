        ...
        <div class="bg-white rounded-xl border border-slate-200 p-6">
            <form method="POST" action="{{ route('deliverables.store') }}" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label for="work_log_id" class="block text-sm font-medium text-slate-700 mb-1">Registro de trabajo relacionado</label>
                        <select name="work_log_id" id="work_log_id"
                                class="w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                            <option value="">— Ninguno —</option>
                            @if ($deliverable->work_log_id)
                                <option value="{{ $deliverable->work_log_id }}" selected>{{ $deliverable->workLog->date->format('d/m/Y') }} · {{ $deliverable->workLog->activity }}</option>
                            @endif
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="version" id="version" maxlength="30" value="{{ old('version', $deliverable->version) }}"
                               placeholder="p. ej. 1.0" class="w-full rounded-lg border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        <label for="file" class="block text-sm font-medium text-slate-700 mb-1">Archivo <span class="text-slate-400 font-normal">(opcional, máx. {{ (int) env('MAX_UPLOAD_MB', 15) }} MB)</span></label>
                ...
                <div class="mt-6 flex items-center gap-3">
                    ...
                    <a href="{{ $deliverable->work_log_id ? route('work-logs.show', $deliverable->work_log_id) : ($deliverable->project_id ? route('projects.show', $deliverable->project_id) : route('projects.index')) }}"
